...
export class VideoPostComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.videoPostService
      ...
      .subscribe(
        (res: HttpResponse<IVideoPost[]>) => this.paginateVideoPosts(res.body, res.headers),
  ...
  trackId(index: number, item: IVideoPost) {
    return item.id;
  }
  ...
  registerChangeInVideoPosts() {
    this.eventSubscriber = this.eventManager.subscribe('videoPostListModification', response => this.reset());
  }
  ...
  protected paginateVideoPosts(data: IVideoPost[], headers: HttpHeaders) {
