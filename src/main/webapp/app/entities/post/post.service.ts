...
export class PostService {
  ...
  protected convertDateFromClient(post: IPost): IPost {
    const copy: IPost = Object.assign({}, post, {
      dateAdded: post.dateAdded != null && post.dateAdded.isValid() ? post.dateAdded.format(DATE_FORMAT) : null,
      dateApproved: post.dateApproved != null && post.dateApproved.isValid() ? post.dateApproved.format(DATE_FORMAT) : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((post: IPost) => {
        post.dateAdded = post.dateAdded != null ? moment(post.dateAdded) : null;
        post.dateApproved = post.dateApproved != null ? moment(post.dateApproved) : null;
      });
    }
    return res;
  }
