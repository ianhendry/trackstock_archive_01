...
/**
 * REST controller for managing {@link com.jugglerapps.stocktrack.domain.Comment}.
 */
@RestController
@RequestMapping("/api")
public class CommentResource {
     ...
     */
    @GetMapping("/comments")
    public ResponseEntity<List<Comment>> getAllComments(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder, @RequestParam(required = false) String filter) {
        if ("post-is-null".equals(filter)) {
            log.debug("REST request to get all Comments where post is null");
            return new ResponseEntity<>(commentService.findAllWherePostIsNull(),
                    HttpStatus.OK);
        }
